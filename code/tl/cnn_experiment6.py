...
##### Users have been tested with Mobiact data.
...
N_FEATURES = 3
N_OUTPUTS = None
RESULTS_FILE = 'results/experiment6/results_cnn.csv'
...
def preprocess_data(df, df_test, user):
    ...
    X_train_t = X_train_t.reshape(X_train_t.shape[0], X_train_t[0].shape[0], X_train_t[0].shape[1], 1)
    X_test_t = X_test_t.reshape(X_test_t.shape[0], X_test_t[0].shape[0], X_test_t[0].shape[1], 1)
...
def main():
    ...
    for user in users:
        ...
        model_path = 'models/mobiact/cnn/base_model_{}.h5'.format(user)
        ...
        x = Dense(len(set(y_train)), activation='softmax', name="dense_b")(x)
        ...
        model_t = Sequential()
        model_t.add(Conv2D(16, (2, 2), activation='relu', input_shape=X_train[0].shape))
        model_t.add(Dropout(0.1))

        model_t.add(Conv2D(32, (2, 2), activation='relu'))
        ...
        model_t.add(Dense(len(set(y_train)), activation='softmax'))

        model_t.compile(optimizer=Adam(learning_rate=0.001), loss='sparse_categorical_crossentropy', metrics=['accuracy'])
        # fit network
        #model_t.fit(X_train_t, y_train_t, epochs=20, batch_size=batch_size, verbose=1)
        model_t.fit(X_train_t, y_train_t, epochs=20, verbose=1)
