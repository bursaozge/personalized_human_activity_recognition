...
RESULTS_FILE = 'results/experiment6/results_lstm.csv'
...
def main():
    ...
    for user in users:
        ...
        model_path = 'models/mobiact/lstm/base_model_{}.h5'.format(user)
        ...
        if os.path.exists(model_path):
            ...
            model.fit(X_train, y_train, epochs=20, batch_size=64, validation_split=0.1, shuffle=True)
        ...
        model_t.add(keras.layers.Dropout(rate=0.5))
        model_t.add(keras.layers.Dense(units=128, activation='relu'))
        model_t.add(keras.layers.Dense(y_train.shape[1], activation='softmax'))
        model_t.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['acc'])
        # fit network
        model_t.fit(X_train_t, y_train_t, epochs=20, batch_size=64, validation_split=0.1, shuffle=True)
